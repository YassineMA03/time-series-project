...
## Design Decisions & Hyperparameter Rationale

### Class weighting strategy

LSST exhibits a 15× imbalance between the most and least frequent classes. Full inverse-frequency weighting ($w_c \propto 1/f_c$) over-penalises the majority class and destabilises training. Sqrt-damping ($w_c \propto 1/\sqrt{f_c}$) provides a softer trade-off that improves minority-class recall while maintaining majority-class precision. This is combined with `label_smoothing=0.1` to prevent overconfidence on small classes.

### Instance normalisation

Raw LSST photometric flux measurements span orders of magnitude across object types. Per-instance z-score normalisation (mean/std over the time axis, per channel) removes the DC offset and scale variations that are uninformative for classification, exposing the temporal morphology that distinguishes transient types. This is applied to both the FCN pipeline and the Chronos pipeline for consistency.

### Cosine warmup schedule

The cosine schedule provides monotonically decreasing learning rate after an initial warmup, which empirically stabilises early training (where gradients are large) and allows fine-grained convergence in later epochs. The 5 % warmup fraction is a conservative default appropriate for datasets of this size.

### Multi-pooling over Chronos patch embeddings

Chronos produces a variable number of patch embeddings $T'$ per sequence depending on the patch configuration of T5-Small. A naive approach (e.g., taking the `[CLS]` token or only mean-pooling) discards temporal distributional information. The concatenation of mean, max, and std statistics captures complementary aspects of the temporal activation profile, tripling the effective feature dimension at low computational cost.

### SVM hyperparameters

`C=10` is a moderately aggressive regularisation setting that encourages tight margins; empirically, higher values tend to overfit on high-dimensional embedding spaces. `gamma='scale'` ($\gamma = 1 / (n\_features \times X.var())$) adapts the RBF kernel width to the feature distribution, avoiding the need for manual tuning.
...
- Chronos embeddings are extracted **channel-by-channel**, ignoring cross-channel dependencies. A joint multi-channel embedding (treating the 6 passbands as a single sequence) could capture inter-band correlations relevant to stellar physics.
- The FCN is trained without data augmentation. Standard TSC augmentations (jitter, time-warping, window slicing) could meaningfully improve generalisation on the minority classes.
- SVM inference does not scale gracefully to larger datasets; a neural probe head would be preferable at scale.

**Future directions:**

- Fine-tuning Chronos (PEFT / LoRA) on LSST training labels, rather than using it purely as a frozen feature extractor.
- Ensemble of BaselineFCN and Chronos probes (late fusion of logits).
- Evaluation on additional UEA datasets to assess the generality of the Chronos transfer gap.
...
Wang, Z., Yan, W., & Oates, T. (2017). **Time Series Classification from Scratch with Deep Neural Networks: A Strong Baseline.** *IJCNN 2017.*

---

*Experiments conducted with PyTorch 2.2, scikit-learn 1.4, and chronos-forecasting 1.3.*
